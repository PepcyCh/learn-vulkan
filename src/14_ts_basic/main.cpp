...
class VulkanAppTessellationBasic : public VulkanApp {
public:
    ~VulkanAppTessellationBasic() {
        if (device) {
...
private:
    ...
    void BuildShaderModules() {
        shader_modules["vert"] = VulkanUtil::CreateShaderModule(src_path + "14_ts_basic/shaders/terrain.vert.spv",
            device->logical_device.get());
        shader_modules["tesc"] = VulkanUtil::CreateShaderModule(src_path + "14_ts_basic/shaders/terrain.tesc.spv",
            device->logical_device.get());
        shader_modules["tese"] = VulkanUtil::CreateShaderModule(src_path + "14_ts_basic/shaders/terrain.tese.spv",
            device->logical_device.get());
        shader_modules["frag"] = VulkanUtil::CreateShaderModule(src_path + "14_ts_basic/shaders/terrain.frag.spv",
            device->logical_device.get());
    }
    void BuildTerrainGeometry() {
        std::array<Eigen::Vector3f , 4> vertices = {
            Eigen::Vector3f(-10.0f, 0.0f, -10.0f),
            Eigen::Vector3f(-10.0f, 0.0f,  10.0f),
            Eigen::Vector3f( 10.0f, 0.0f, -10.0f),
            Eigen::Vector3f( 10.0f, 0.0f,  10.0f),
        };
    ...
    void BuildRenderItems() {
        ...
        auto terrain_ritem = std::make_unique<RenderItem>();
        terrain_ritem->obj_index = obj_index++;
        terrain_ritem->n_frame_dirty = n_inflight_frames;
        terrain_ritem->mesh = geometries["terrain_geo"].get();
        terrain_ritem->vertex_buffer = terrain_ritem->mesh->vertex_buffer->buffer.get();
        terrain_ritem->index_buffer = terrain_ritem->mesh->index_buffer->buffer.get();
        terrain_ritem->mat = materials["grass"].get();
        terrain_ritem->tex_transform = MathUtil::Scale({ 5.0f, 5.0f, 1.0f });
        terrain_ritem->n_index = terrain_ritem->mesh->draw_args["terrain"].n_index;
        terrain_ritem->first_index = terrain_ritem->mesh->draw_args["terrain"].first_index;
        terrain_ritem->vertex_offset= terrain_ritem->mesh->draw_args["terrain"].vertex_offset;
        items[static_cast<size_t>(RenderLayer::Opaque)].push_back(terrain_ritem.get());
        render_items.push_back(std::move(terrain_ritem));
    }
    ...
    void BuildGraphicsPipeline() {
        ...
        vk::PipelineTessellationStateCreateInfo tessellation({}, 4);
...
int main() {
    try {
        VulkanAppTessellationBasic app;
        app.Initialize();
        app.MainLoop();
    } catch (const std::runtime_error &e) {
