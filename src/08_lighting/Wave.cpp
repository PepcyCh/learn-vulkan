...
Wave::Wave(int m, int n, float dx, float dt, float speed, float damping) {
    ...
    for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            prev_solution[i * n + j] = { x, 0.0f, z };
            curr_solution[i * n + j] = { x, 0.0f, z };
            normals[i * n + j] = { 0.0f, 1.0f, 0.0f };
            tangents[i * n + j] = { 1.0f, 0.0f, 0.0f };
        }
    }
}
    ...
    if (t >= time_step) {
        ...
        for (int i = 1; i < n_row - 1; i++) {
            for (int j = 1; j < n_col - 1; j++) {
                ...
                normals[i * n_col + j] = Eigen::Vector3f(l - r, 2.0f * spatial_step, b - t).normalized();
                tangents[i * n_col + j] = Eigen::Vector3f(2.0f * spatial_step, r - l, 0.0f).normalized();
            }
        }
    }
}
