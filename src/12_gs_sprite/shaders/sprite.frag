...
layout(set = 1, binding = 0) uniform sampler2DArray diffuse_cis;
...
void main() {
    vec2 texc = vec2(mat.mat_transform * vec4(fin.texc, 0.0f, 1.0f));
    vec4 diffuse_tex = texture(diffuse_cis, vec3(texc, fin.prim_id % 3));
    ...
    vec4 diffuse = mat.albedo * diffuse_tex;
    Material mat = { diffuse, mat.fresnel_r0, shininess };
