...
struct WavesTexTransform {
    Eigen::Matrix4f height_trans0;
    Eigen::Matrix4f height_trans1;
    Eigen::Matrix4f normal_trans0;
    Eigen::Matrix4f normal_trans1;
    Eigen::Vector2f height_scale;
};
...
struct FrameResources {
    FrameResources(const vku::Device *device, vk::DescriptorPool descriptor_pool,
        ...
        vk::DescriptorSetLayout waves_set_layout, size_t n_waves);
    ...
    std::vector<vk::DescriptorSet> waves_set;
    ...
    std::unique_ptr<HostVisibleBuffer<WavesTexTransform>> waves_ub;
};