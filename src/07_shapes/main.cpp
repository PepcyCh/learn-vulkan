    ...
    void WriteDescriptorSets() {
        ...
        for (size_t i = 0; i < n_inflight_frames; i++) {
            for (size_t j = 0; j < render_items.size(); j++) {
                buffer_infos[p] = vk::DescriptorBufferInfo(frame_resources[i]->obj_ub->Buffer(),
            ...
            buffer_infos[p] = vk::DescriptorBufferInfo(frame_resources[i]->pass_ub->Buffer(), 0, pass_ub_size);
